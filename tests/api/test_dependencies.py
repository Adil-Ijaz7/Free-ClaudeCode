...
def _make_mock_settings(**overrides):
    ...
    mock = MagicMock()
    mock.model = "open_router/anthropic/claude-3.5-sonnet"
    mock.provider_type = "open_router"
...
async def test_get_provider_passes_http_timeouts_from_settings():
    ...
    with (
        ...
        patch("providers.anthropic_messages.httpx.AsyncClient") as mock_httpx,
    ):
        mock_settings.return_value = _make_mock_settings(
        ...
        timeout = mock_httpx.call_args.kwargs["timeout"]
...
async def test_get_provider_passes_proxy_from_settings():
    ...
    with (
        ...
        mock_settings.return_value = _make_mock_settings(
            open_router_proxy="http://proxy.example:8080"
        )
...
async def test_get_provider_for_type_caches():
    ...
    with patch("api.dependencies.get_settings") as mock_settings:
        ...
        p1 = get_provider_for_type("open_router")
        p2 = get_provider_for_type("open_router")
...
async def test_cleanup_provider_cleans_all():
    ...
    with patch("api.dependencies.get_settings") as mock_settings:
        ...
        openrouter = get_provider_for_type("open_router")

        assert isinstance(openrouter, OpenRouterProvider)

        openrouter._client = AsyncMock()
...
def test_resolve_provider_per_app_uses_separate_registries() -> None:
    ...
    with patch("api.dependencies.get_settings") as mock_settings:
        ...
        p1 = resolve_provider(
            "open_router", app=cast(Starlette, app1), settings=settings
        )
        p2 = resolve_provider(
            "open_router", app=cast(Starlette, app2), settings=settings
        )
        assert isinstance(p1, OpenRouterProvider)
...
def test_resolve_provider_missing_registry_raises_service_unavailable() -> None:
    ...
    with patch("api.dependencies.get_settings") as mock_settings:
        ...
        with pytest.raises(
            ...
            resolve_provider("open_router", app=cast(Starlette, app), settings=settings)
...
def test_resolve_provider_unrelated_value_error_is_not_unknown_provider_log() -> None:
    ...
    with (
        ...
        deps.resolve_provider("open_router", app=None, settings=_make_mock_settings())
