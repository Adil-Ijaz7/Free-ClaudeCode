...
def settings():
    settings = Settings()
    settings.model = "open_router/fallback-model"
...
def test_model_router_resolves_default_model(settings):
    ...
    assert resolved.provider_model_ref == "open_router/fallback-model"
...
def test_model_router_applies_haiku_override(settings):
    settings.model_haiku = "open_router/qwen2.5-7b"
    ...
    assert routed.resolved.provider_model_ref == "open_router/qwen2.5-7b"


def test_model_router_applies_sonnet_override(settings):
    settings.model_sonnet = "open_router/meta/llama-3.3-70b-instruct"
    ...
    assert (
        routed.resolved.provider_model_ref == "open_router/meta/llama-3.3-70b-instruct"
    )
...
def test_model_router_routes_prefixed_provider_model_directly(settings):
    ...
    assert routed.resolved.original_model == "open_router/deepseek/deepseek-chat"
    assert routed.resolved.provider_id == "open_router"
    assert routed.resolved.provider_model == "deepseek/deepseek-chat"
    assert routed.resolved.provider_model_ref == "open_router/deepseek/deepseek-chat"


def test_model_router_routes_gateway_encoded_provider_model_directly(settings):
    routed = ModelRouter(settings).resolve_messages_request(
        MessagesRequest(
            model="anthropic/open_router/deepseek-ai/deepseek-v4-pro",
            max_tokens=100,
            messages=[Message(role="user", content="hello")],
        )
    )
    ...
    assert (
        ...
        == "anthropic/open_router/deepseek-ai/deepseek-v4-pro"
    )
    assert routed.resolved.provider_id == "open_router"
...
def test_model_router_routes_no_thinking_gateway_model_directly(settings):
    ...
    routed = ModelRouter(settings).resolve_messages_request(
        MessagesRequest(
            model="claude-3-freecc-no-thinking/open_router/deepseek-ai/deepseek-v4-pro",
            max_tokens=100,
            messages=[Message(role="user", content="hello")],
        )
    )
    ...
    assert (
        routed.resolved.original_model
        == "claude-3-freecc-no-thinking/open_router/deepseek-ai/deepseek-v4-pro"
    )
    assert routed.resolved.provider_id == "open_router"
    assert routed.resolved.provider_model == "deepseek-ai/deepseek-v4-pro"
