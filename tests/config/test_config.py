...
class TestSettings:
    ...
    def test_default_values(self, monkeypatch):
        ...
        settings = Settings()
        assert settings.model == "open_router/openai/gpt-oss-120b:free"
...
class TestPerModelMapping:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {"MODEL": "open_router/meta/llama3-70b-instruct"},
                "open_router/meta/llama3-70b-instruct",
                None,
            ),
            (
                {
    ...
    def test_model_sonnet_from_env(self, monkeypatch):
        ...
        monkeypatch.setenv("MODEL_SONNET", "open_router/meta/llama-3.3-70b-instruct")
        s = Settings()
        assert s.model_sonnet == "open_router/meta/llama-3.3-70b-instruct"

    def test_model_haiku_from_env(self, monkeypatch):
        """MODEL_HAIKU env var is loaded."""
        from config.settings import Settings

        monkeypatch.setenv("MODEL_HAIKU", "open_router/qwen2.5-7b")
        s = Settings()
    ...
    def test_resolve_model_sonnet_override(self):
        ...
        s = Settings()
        s.model_sonnet = "open_router/meta/llama-3.3-70b-instruct"
        assert (
            s.resolve_model("claude-sonnet-4-20250514")
            == "open_router/meta/llama-3.3-70b-instruct"
        )
        assert (
    ...
    def test_resolve_model_haiku_override(self):
        ...
        s = Settings()
        s.model_haiku = "open_router/qwen2.5-7b"
        assert s.resolve_model("claude-3-haiku-20240307") == "open_router/qwen2.5-7b"
        assert s.resolve_model("claude-3-5-haiku-20241022") == "open_router/qwen2.5-7b"
        assert s.resolve_model("claude-haiku-4-20250514") == "open_router/qwen2.5-7b"
    ...
    def test_resolve_model_fallback_when_override_not_set(self):
        ...
        s = Settings()
        s.model = "open_router/fallback-model"
        # No model overrides set
        assert s.resolve_model("claude-opus-4-20250514") == "open_router/fallback-model"
        assert (
            s.resolve_model("claude-sonnet-4-20250514") == "open_router/fallback-model"
        )
        assert (
            s.resolve_model("claude-3-haiku-20240307") == "open_router/fallback-model"
        )
    ...
    def test_resolve_model_unknown_model_falls_back(self):
        ...
        assert s.resolve_model("claude-2.1") == "open_router/fallback-model"
        assert s.resolve_model("some-unknown-model") == "open_router/fallback-model"
    ...
    def test_parse_model_name(self):
        ...
        assert Settings.parse_model_name("open_router/deepseek/r1") == "deepseek/r1"
